...
readonly TWILIO_ACCOUNT_SID_SECRET="TWILIO_ACCOUNT_SID"
readonly TWILIO_AUTH_TOKEN_SECRET="TWILIO_AUTH_TOKEN"
readonly TWILIO_VERIFY_SERVICE_SID_SECRET="TWILIO_VERIFY_SERVICE_SID"
...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --cpu=1 \
  --timeout=300 \
  --update-env-vars "^~^NODE_ENV=production~HOST=0.0.0.0~PUBLIC_FRONTEND_ORIGIN=$PUBLIC_FRONTEND_ORIGINS~BUSINESS_FRONTEND_ORIGIN=$BUSINESS_FRONTEND_URL~FRONTEND_BASE_URL=$FRONTEND_URL~CATALOG_GOOGLE_REDIRECT_URL=$FRONTEND_URL~FIREBASE_AUTH_PROJECT_ID=$PROJECT_ID~GCS_CATALOG_BUCKET=$PRIVATE_BUCKET~GCS_CATALOG_PREFIX=~GCS_CATALOG_PUBLIC_ASSET_BUCKET=$PUBLIC_BUCKET~CATALOG_PUBLIC_ASSET_BASE_URL=https://storage.googleapis.com/$PUBLIC_BUCKET~CATALOG_SIGNED_URL_TTL_SECONDS=900~CATALOG_OTP_DELIVERY_ENABLED=true~CATALOG_OTP_PROVIDER=twilio~CATALOG_OTP_TTL_SECONDS=600~CATALOG_OTP_RESEND_AFTER_SECONDS=60~CATALOG_OTP_MAX_ATTEMPTS=5~GCS_PRODUCT_BUCKET=$PRODUCT_BUCKET~GCS_PRODUCT_THUMBNAIL_BUCKET=$PUBLIC_BUCKET~PRODUCT_FIRESTORE_DATABASE_ID=$FIRESTORE_DATABASE_ID~PRODUCT_ASSET_DELIVERY=signed~PRODUCT_SIGNED_URL_TTL_SECONDS=3600~GOOGLE_CLIENT_ID=$GOOGLE_CLIENT_ID~BUSINESS_UI_GOOGLE_CLIENT_ID=$GOOGLE_CLIENT_ID~FIRESTORE_DATABASE_ID=$FIRESTORE_DATABASE_ID~BUSINESS_UI_ALLOWED_GOOGLE_EMAILS=$BUSINESS_ALLOWED_EMAILS~RECAPTCHA_ENTERPRISE_PROJECT_ID=$RECAPTCHA_PROJECT_ID~RECAPTCHA_ENTERPRISE_SITE_KEY=$RECAPTCHA_SITE_KEY~INTERNAL_ADMIN_API_ENABLED=false~INTERNAL_ADMIN_SERVICE_ONLY=false" \
  --remove-env-vars "BACKEND_ADMIN_TOKEN,FRONTEND_ORIGIN" \
  --update-secrets "BUSINESS_UI_CSRF_SECRET=$BUSINESS_CSRF_SECRET:latest,TWILIO_ACCOUNT_SID=$TWILIO_ACCOUNT_SID_SECRET:latest,TWILIO_AUTH_TOKEN=$TWILIO_AUTH_TOKEN_SECRET:latest,TWILIO_VERIFY_SERVICE_SID=$TWILIO_VERIFY_SERVICE_SID_SECRET:latest" \
